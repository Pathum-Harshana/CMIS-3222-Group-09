...
try {
    $stmt = $pdo->prepare("INSERT INTO posts (user_id, content, mood) VALUES (:uid,:content,:mood)");
    $stmt->execute([
        ":uid" => $user["id"],
        ":content" => $content,
        ":mood" => ($mood !== "" ? $mood : null)
    ...
    $get = $pdo->prepare("SELECT id,user_id,content,mood,is_flagged,is_reviewed,created_at FROM posts WHERE id=:id LIMIT 1");
    $get->execute([":id"=>$id]);
